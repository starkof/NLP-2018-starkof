...
def preProcessing(*files):
    ...
    with open("vectorizer2.pickle",'wb') as f:
...
def toVector(*files):
    ...
    vectorizer = load('vectorizer2.pickle')
...
def logRegression(tup):
    ...
    with open("regressor2.pickle",'wb') as f:
...
def main(argv):
    ...
    writeFile = open('lr-n.txt', 'w')
    exist = False
    try:
        f = open('regressor2.pickle', 'rb')
        f.close()
        exist = True
    except FileNotFoundError:
        pass
    if exist is True:
        model = load('regressor2.pickle')
