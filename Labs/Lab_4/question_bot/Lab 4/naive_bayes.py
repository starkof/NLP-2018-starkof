...
def preProcessing(*files):
    ...
    with open("vectorizer3.pickle",'wb') as f:
...
def toVector(*files):
    ...
    vectorizer = load('vectorizer3.pickle')
...
def naiveBayes(tup):
    nb = MultinomialNB()
    nb.fit(tup[0], tup[2])
    with open("bayes1.pickle",'wb') as f:
        pickle.dump(nb, f)
...
def main(argv):
    ...
    writeFile = open('nb-u.txt', 'w')
    exist = False
    try:
        f = open('bayes1.pickle', 'rb')
        f.close()
        exist = True
    except FileNotFoundError:
        pass
    if exist is True:
        model = load('bayes1.pickle')
